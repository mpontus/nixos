    ...
    xdg.dataFile."backgrounds/xfce".source =
      let
        wallpapers = [
          ...
          { prefix = "budgie"; package = pkgs.budgie-backgrounds; }
          { prefix = "cosmic"; package = pkgs.cosmic-wallpapers; }
          { prefix = "gnome"; package = pkgs.gnome-backgrounds; }
          { prefix = "kde"; package = pkgs.kdePackages.plasma-workspace-wallpapers; }
          { prefix = "elementary"; package = pkgs.pantheon.elementary-wallpapers; }
          { prefix = "pop-hp"; package = pkgs.pop-hp-wallpapers; }
          { prefix = "pop"; package = pkgs.pop-wallpapers; }
          { prefix = "system76"; package = pkgs.system76-wallpapers; }
        ];
        gallery = pkgs.runCommand "xfce-wallpapers" {
          ...
          ${lib.concatMapStringsSep "\n" ({ prefix, package }: ''
            for directory in "${package}/share/backgrounds" "${package}/share/wallpapers"; do
              ...
              find "$directory" -type f \
                ...
                -exec sh -c 'directory=$1; shift; for image; do relative="''${image#"$directory"/}"; name=$(printf "%s" "$relative" | tr "/" "-"); ln -s "$image" "$out/share/backgrounds/xfce/${prefix}-$name"; done' sh "$directory" {} +
            done
          '') wallpapers}
        '';
