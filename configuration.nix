...

{
  ...
  let
    i3CinnamonSession = pkgs.runCommand "i3-cinnamon-session" {
      passthru.providedSessions = [ "i3-cinnamon" ];
    } ''
      mkdir -p $out/bin $out/share/applications $out/share/cinnamon-session/sessions $out/share/xsessions
  
      install -Dm755 ${pkgs.writeShellScript "cinnamon-session-i3" ''
        exec ${pkgs.cinnamon-session}/bin/cinnamon-session --session i3-cinnamon "$@"
      ''} $out/bin/cinnamon-session-i3
  
      install -Dm755 ${pkgs.writeShellScript "i3-cinnamon" ''
        if [ -n "$DESKTOP_AUTOSTART_ID" ]; then
          ${pkgs.dbus}/bin/dbus-send --print-reply --session --dest=org.gnome.SessionManager \
            /org/gnome/SessionManager org.gnome.SessionManager.RegisterClient \
            string:i3-cinnamon string:"$DESKTOP_AUTOSTART_ID" || true
        fi
        [ -e "$HOME/.Xresources" ] && ${pkgs.xorg.xrdb}/bin/xrdb -merge "$HOME/.Xresources"
        exec ${pkgs.i3}/bin/i3 -c /etc/i3/config
      ''} $out/bin/i3-cinnamon
  
      install -Dm644 ${pkgs.writeText "i3-cinnamon.desktop" ''
        [Desktop Entry]
        Type=Application
        Name=i3-cinnamon
        NoDisplay=true
        Comment=i3 window manager with Cinnamon session services
        Exec=$out/bin/i3-cinnamon
        X-GNOME-WMName=i3-cinnamon
        X-GNOME-Autostart-Phase=WindowManager
        X-GNOME-Provides=windowmanager
        X-GNOME-Autostart-Notify=false
      ''} $out/share/applications/i3-cinnamon.desktop
  
      install -Dm644 ${pkgs.writeText "i3-cinnamon.session" ''
        [Cinnamon Session]
        Name=i3-cinnamon
      ...
      ''} $out/share/cinnamon-session/sessions/i3-cinnamon.session
  
      install -Dm644 ${pkgs.writeText "i3-cinnamon-xsession.desktop" ''
        [Desktop Entry]
        Name=Cinnamon + i3
        Comment=This session logs you into Cinnamon with i3 as the window manager
        Exec=$out/bin/cinnamon-session-i3
        TryExec=$out/bin/i3-cinnamon
        Type=Application
      ''} $out/share/xsessions/i3-cinnamon.desktop
  
      substituteInPlace $out/share/applications/i3-cinnamon.desktop --replace-fail '$out' "$out"
      substituteInPlace $out/share/xsessions/i3-cinnamon.desktop --replace-fail '$out' "$out"
    '';
  in {
    ...
    services.xserver.windowManager.i3 = {
      enable = true;
      configFile = pkgs.writeText "i3-cinnamon-config" ''
        set $mod Mod4
        ...
        exec --no-startup-id xfce4-terminal --title 'Cinnamon+i3 VM' --command 'bash -lc "echo Cinnamon+i3 VM session is running.; echo Super+Enter opens a terminal.; exec bash"'
  
        bindsym $mod+Return exec xfce4-terminal
        bindsym $mod+d exec dmenu_run
        bindsym $mod+Shift+q kill
        bindsym $mod+Shift+r restart
        bindsym $mod+Shift+e exec "cinnamon-session-quit --logout --no-prompt"
        ...
        bindsym $mod+p exec cinnamon-settings
    ...
    services.displayManager.sessionPackages = [ i3CinnamonSession ];
    ...
    services.displayManager.defaultSession = lib.mkForce "i3-cinnamon";
    ...
    security.pam.services.cinnamon-screensaver.allowNullPassword = true;
  };
  hardware.bluetooth.enable = true;
