...
This module keeps GNOME available as a GDM fallback while making the tested Xorg XFCE + XMonad session the default login.

*** Error dialogs

XMonad surfaces config compile errors through ~xmessage~. GDM merges ~~/.Xresources~ at login; Athena widgets need wildcarded lowercase resources to reach child widgets.
...
#+begin_src nix :tangle xfce-xmonad.nix :noweb-ref xfce-xmonad-module
{ lib, pkgs, ... }:
let
